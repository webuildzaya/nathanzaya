...
export default function InstructorsPage() {
  ...
  const createInstructor = useMutation({
    mutationFn: async (payload: { fullName: string; phone?: string; email?: string; password?: string }) => {
                ...
                {instructors.map((i: any) => (
                  <tr key={i.id} className="hover:bg-gray-50/50 transition-colors group">
                    ...
                    <td className="px-6 py-4">
                        ...
                        {i.user?.email && <div className="text-[10px] font-medium text-blue-600 uppercase tracking-tight">{i.user.email}</div>}
                    </td>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-1.5">
                ...
                <input
                  ...
                  className="w-full h-12 px-4 border border-gray-200 rounded-xl text-base focus:outline-none focus:ring-2 focus:ring-blue-600 bg-white placeholder-gray-300"
                  placeholder="e.g. 080123"
                />
              </div>
              <div className="space-y-1.5 opacity-40">
                 <div className="h-12 border-l border-gray-200 hidden md:block" />
              </div>
          </div>

          <div className="border-t border-gray-100 pt-4">
              <p className="text-[10px] font-bold text-gray-400 uppercase tracking-[0.2em] mb-4">Login Credentials (Optional)</p>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-1.5">
