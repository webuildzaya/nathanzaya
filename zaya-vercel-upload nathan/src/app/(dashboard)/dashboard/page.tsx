...
export default async function DashboardPage() {
  ...
  const user = await prisma.user.findUnique({
    ...
    select: { schoolId: true, fullName: true, role: true },
  })

  if (!user) redirect('/login')

  const { schoolId, fullName } = user
  ...
  const [studentCount, lessonCount, paymentAgg] = await Promise.all([
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 bg-blue-100 rounded-full flex items-center justify-center border-2 border-white shadow-sm">
              <span className="text-blue-700 font-bold text-sm">{fullName[0].toUpperCase()}</span>
            </div>
          </div>
        </div>
      </header>
      ...
      <div className="relative overflow-hidden bg-gradient-to-r from-blue-800 to-[#0F1B4C] rounded-3xl p-8 md:p-12 text-white shadow-xl shadow-blue-900/10">
        <div className="relative z-10 max-w-2xl">
          ...
          <h2 className="text-3xl md:text-4xl font-bold mb-4">
            You have {studentCount} active students enrolled this session.
          </h2>
          ...
          <div className="flex flex-wrap gap-4">
            <button className="px-6 py-3 bg-white text-blue-700 rounded-xl font-bold hover:bg-blue-50 transition-all active:scale-95 shadow-lg shadow-white/10">
              View reports
            </button>
            <button className="px-6 py-3 border border-white/30 rounded-xl font-semibold hover:bg-white/10 transition-all active:scale-95">
              Add new student
            </button>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm transition-hover hover:shadow-md">
          ...
          <div className="pt-4 border-t border-gray-50 flex items-center justify-between">
             <span className="text-xs font-bold text-green-600 bg-green-50 px-2 py-1 rounded-full">+12% from last month</span>
        ...
        <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm transition-hover hover:shadow-md">
          ...
          <div className="pt-4 border-t border-gray-50 flex items-center justify-between">
             <span className="text-xs font-bold text-blue-600 bg-blue-50 px-2 py-1 rounded-full">Next lesson: Today 4 PM</span>
             <svg className="w-4 h-4 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
             </svg>
          </div>
        ...
        <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm transition-hover hover:shadow-md">
          ...
          <div className="text-5xl font-extrabold text-gray-900 mb-1 tracking-tight">₦{(totalRevenue / 1000).toFixed(0)}K</div>
          <div className="text-sm text-gray-500 font-bold uppercase tracking-wider mb-4">Total Revenue</div>
          <div className="pt-4 border-t border-gray-50 flex items-center justify-between">
             <span className="text-xs font-bold text-orange-600 bg-orange-50 px-2 py-1 rounded-full">Outstanding: ₦{(totalRevenue * 0.2 / 1000).toFixed(0)}K</span>
             <svg className="w-4 h-4 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
             </svg>
          </div>
